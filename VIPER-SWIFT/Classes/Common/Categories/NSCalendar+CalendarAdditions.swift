...
//        let endDay = nextDay.dateByAddingTimeInterval(-1)
    ...
    func isDate(_ date: Date, duringSameWeekAsDate: Date) -> Bool {
        ...
        let duringSameWeekComponents = self.dateComponents([.weekOfYear], from: duringSameWeekAsDate)
    ...
    func isDate(_ date: Date, duringWeekAfterDate: Date) -> Bool {
        ...
        let dateComponents = self.dateComponents([.weekOfYear], from: date)
        let nextWeekComponents = self.dateComponents([.weekOfYear], from: nextWeek)
